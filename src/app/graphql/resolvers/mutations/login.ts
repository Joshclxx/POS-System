...
export const loginMutation = {
    Mutation: {
        loginAndRecord: async (_: unknown, args: {data: {email: string, password: string}}, context: GraphQLContext) => {
            ...
            const userDataFormat = async () => {
                ...
                const loginSession =await context.prisma.loginHistory.create({data: {userId: user.id, timeIn: new Date()}})
        ...
        updateLoginRecord: async (_: unknown, {userId} : {userId: string}, context: GraphQLContext) => {

            const updateLogin = async () => {
                const loginHistory =  await context.prisma.loginHistory.findFirst({where: {userId, timeOut: null}, orderBy: {timeIn: "desc"}});

                if (!loginHistory) {
                  throw new GraphQLError("No active login session found for this user.", {
                    extensions: {
                ...
                return await context.prisma.loginHistory.update({where: {id: loginHistory.id}, data: {timeOut: new Date()}})
            }
