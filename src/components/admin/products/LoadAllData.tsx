...
const DataLoader = () => {
  const { data: productData, loading: loadingProducts, error: errorProducts } = useQuery(GET_ALL_PRODUCTS);
  const { data: categoryData, loading: loadingCategories, error: errorCategories } = useQuery(GET_ALL_CATEGORIES);
  ...
  useEffect(() => {
    if (productData?.getAllProducts) {
      const formattedProducts = productData.getAllProducts.map((product: RawProduct) => {
        const prices = {
    ...
    if (categoryData?.getAllCategories) {
      const formattedMenus = categoryData.getAllCategories.map((menu: RawCategory) => menu.name);
